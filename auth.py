"""
User Authentication Module for Rakshak
...
class UserDatabase:
    ...
    def __init__(self, db_path='risk_module.db'):
        # Fix: Use local path, don't look in parent dir
        self.db_path = db_path
        self.init_database()
    
    def init_database(self):
        """Initialize users table via centralized Database class"""
        # We rely on database.py to create tables now
        pass
    ...
    def create_user(self, username, email, password, role='passenger', phone=None):
        ...
        # Use database.py's method
        user_id = db.create_user(username, email, password_hash, role, phone)
        db.close()
    ...
    def get_user_by_id(self, user_id):
        ...
        if row:
            # Row is a tuple from database.py (SELECT *), usually:
            # id, username, email, password_hash, role, phone, created_at, (sometimes is_active missing in old schema?)
            # Our updated database.py schema for users:
